...
#	infile ='/u/user/yeobi97/JKPS_WZG/WZG/lightend_ML/run/Eval/'+cha+'_prediction.csv'
	infile = '/u/user/yeobi97/JKPS_WZG/WZG/final_trial/run/Eval/'+cha+'_prediction.csv'
	df = pd.read_csv(infile)
...
###############################################################################################
	...
	plt.yscale('log')
	plt.savefig("{0}_DNN_score_b100.png".format(cha))
	plt.show()
	plt.close()
...
def sigma(cha,sf):
	...
	print('thisis'+cha+'it',max(arr_sig))
